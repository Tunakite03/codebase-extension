...
export function writeCodebaseDir(workspace: string): void {
  ...
  const project = state.stats.projects.find(
    (p) => normalizePath(p.path) === normalizedWs,
  );
...
export async function setupAgentConfigs(workspace: string): Promise<void> {
  ...
  const existing = targets.filter((t) =>
    fs.existsSync(path.join(workspace, t)),
  );
...
`;
  ...
  const writeManagedMarkdown = (dir: string, file: string, content: string) => {
    fs.mkdirSync(dir, { recursive: true });
    const filePath = path.join(dir, file);
  ...
  const upsertMarkdown = (dir: string, file: string, content: string) => {
    ...
    if (fs.existsSync(filePath)) {
      ...
      if (markerIdx !== -1) {
        ...
        const afterMarker = existing.substring(
          markerIdx + CBM_SECTION_MARKER.length,
        );
        const nextHeadingMatch = afterMarker.match(/\n(?=## (?!#))/);
        const sectionEnd = nextHeadingMatch
          ? markerIdx +
            CBM_SECTION_MARKER.length +
            (nextHeadingMatch.index as number)
          : existing.length;
  ...
  writeManagedMarkdown(path.join(workspace, ".zed"), "rules.md", zedRules);

  vscode.window.showInformationMessage(
    `${DISPLAY_NAME}: Agent configs written to .vscode/, .github/, .github/instructions/, .claude/, .cursor/, and .zed/`,
  );
}
