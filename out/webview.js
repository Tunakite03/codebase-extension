...
exports.CBMWebviewProvider = void 0;
const vscode = __importStar(require("vscode"));
const path = __importStar(require("path"));
const types_1 = require("./types");
const state_1 = require("./state");
class CBMWebviewProvider {
    ...
    _getHtml() {
        ...
        const uptime = state_1.state.stats.startedAt ? formatUptime(Date.now() - state_1.state.stats.startedAt.getTime()) : '\u2014';
        const lastIdx = state_1.state.stats.lastIndexed ? timeAgo(state_1.state.stats.lastIndexed) : '\u2014';
        const fmt = (n) => (n > 0 ? n.toLocaleString() : '\u2014');
        const binaryFound = state_1.state.resolvedBinary !== null;
        const running = state_1.state.isRunning;
        const indexing = state_1.state.stats.isIndexing;
        // --- Status indicator ---
        let statusDot;
        let statusLabel;
        if (!binaryFound) {
        ...
        const metricsHtml = `
         <div class="metrics">
            <div class="metric-card">
               <div class="metric-value">${fmt(state_1.state.stats.nodes)}</div>
               <div class="metric-label">Nodes</div>
            </div>
            <div class="metric-card">
               <div class="metric-value">${fmt(state_1.state.stats.edges)}</div>
        ...
        // --- Project cards ---
        const projectsHtml = state_1.state.stats.projects.length > 0
            ? state_1.state.stats.projects
                .map((p) => `
         ...
         <div class="project-card ${running ? '' : 'project-inactive'}">
            ...
            <div class="project-metrics">
               <span class="pm">${fmt(state_1.state.stats.nodes)} <em>nodes</em></span>
               <span class="pm-sep">&middot;</span>
               <span class="pm">${fmt(state_1.state.stats.edges)} <em>edges</em></span>
            </div>
            ${state_1.state.stats.lastIndexed ? `<div class="project-last">Last indexed ${lastIdx}</div>` : ''}
         </div>`
...
<div class="header">
   <div class="header-left">
      ...
      <div>
         <div class="header-title">${types_1.DISPLAY_NAME}</div>
...
exports.CBMWebviewProvider = CBMWebviewProvider;
CBMWebviewProvider.viewType = 'contextEngine.panel';
// --- Helpers ---
function escHtml(s) {
